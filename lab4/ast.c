...
node *ast_allocate(node_kind kind, ...) {
  ...
  case FUNCTION_NODE:
    ...
    ast->func.ret_type = NULL; // set when checking types in semantic.c
    break;
  ...
  case CONSTRUCTOR_NODE:
	...
	ERROR("Can't apply constructor to scalar type");
    }
    break;
...
void ast_free(node *ast) {
    ...
    case SCOPE_NODE:
        ast_free(ast->scope.decs);
        ast_free(ast->scope.stmts);
        break;

    case STATEMENTS_NODE:
        ast_free(ast->statements.next);
        ast_free(ast->statements.stmt);
        break;

    case DECLARATIONS_NODE:
        ast_free(ast->declarations.next);
        ast_free(ast->declarations.dec);
        break;

    case DECLARATION_NODE:
        //ast->dec.name = va_arg(args, char *);
        ast_free(ast->dec.type);
        ast_free(ast->dec.init);
        break;
    ...
    case IF_STATEMENT_NODE:
        ast_free(ast->if_st.expr);
        ast_free(ast->if_st.stmt1);
        ast_free(ast->if_st.stmt2);
        break;

    case ASSIGNMENT_NODE:
        ast_free(ast->assignment.var);
        ast_free(ast->assignment.expr);
        ast_free(ast->assignment.type);
        break;

    case EXPRESSION_NODE:
    case VAR_NODE:
    case NESTED_SCOPE_NODE:
        ast_free(ast->expr.ex);
        break;

    case UNARY_EXPRESSION_NODE:
        ast_free(ast->unary_expr.right);
        ast_free(ast->unary_expr.type);
        break;

    case BINARY_EXPRESSION_NODE:
        ast_free(ast->binary_expr.left);
        ast_free(ast->binary_expr.right);
        ast_free(ast->binary_expr.type);
        break;
    ...
    case INDEX_NODE:
        //ast->index.name = va_arg(args, char *);
        ast_free(ast->index.type);
        break;

    case FUNCTION_NODE:
        ast_free(ast->func.args);
        ast_free(ast->func.ret_type);
        break;

    case CONSTRUCTOR_NODE:
        ast_free(ast->constr.type);
        ast_free(ast->constr.args);
        break;
    }
